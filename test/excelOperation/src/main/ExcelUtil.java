...
public class ExcelUtil {
   ...
   * @return 返回文件列名的列号
   */
  public static int getColumnNum(String filePath, String columnName) {
    File file = new File(filePath);
    int con = 0;
    ArrayList<ArrayList<Object>> result = ExcelUtil.readExcel(file);
    for (int i = 0; i < result.size(); i++) {
      for (int j = 0; j < result.get(i).size(); j++) {
        if (result.get(i).get(j).equals(columnName)) {
          con = j;
          return con;
        }
      }
  ...
  public static void printColumnNumInfo(String filePath, int columnNum) {
    File file = new File(filePath);
    ArrayList<ArrayList<Object>> result = ExcelUtil.readExcel(file);
    for (int i = 0; i < result.size(); i++) {
      System.out.println(result.get(i).get(columnNum));
    }
  }
  public static void printAllInfo(String filePath) {
    File file = new File(filePath);
    ArrayList<ArrayList<Object>> result = ExcelUtil.readExcel(file);
    for (int i = 0; i < result.size(); i++) {
      for (int j = 0; j < result.get(i).size(); j++) {
          System.out.println("行 "+i+result.get(i)+"   列   "+result.get(i).get(j));
      };
    }
  }
